# -*- coding: utf-8 -*-
"""
Created on Wed Sep 29 16:40:23 2021

@author: Dmitry Trushin
"""
...
def getPagesCount():
    ...
    ads_total = int(''.join(ads_total)) - 20000
    number_of_pages = int(ads_total / 38)
...
def getLinks():
    ...
    for i in range(1, number_of_pages): # Number of pages you want to parse
        ...
        with open('Project_6.csv', 'a') as myfile:
            wr = csv.writer(myfile)
            for elem in cars_from_page_list:
                wr.writerow([elem])
                all_ads.append(elem)
    return all_ads
Parallel(n_jobs = 2)(delayed(getLinks()))